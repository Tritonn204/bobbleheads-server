...
console.log(`Worker ${process.pid} is running`, `listening on port ${port}`);

processPorts[process.pid] = port;
    ...
    io.on("connection", socket => {
        ...
        socket.on('fetchData', async (cb) => {
            ...
            cb({
                ...
                skeleton: data ? data.skeleton : null
            });
        })
        ...
        socket.on('createGame', async(room, cb) => {
            ...
            await pubClient.set('match:' + gameId + ':port', port);
